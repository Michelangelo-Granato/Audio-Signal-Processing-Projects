...
def computeEngEnv(inputFile, window, M, N, H):
    ...
    frameBins = H * np.arange(frames)/float(fs)

    binsFreq = np.arange(N/2 + 1) * float(fs)/N

    index0K = np.where(binsFreq>0)[0][0]
    indexUnder3K = np.where(binsFreq<3000)[0][-1]
    indexOver3K = np.where(binsFreq>3000)[0][0]
    index10K = np.where(binsFreq<10000)[0][-1]

    engEnv = np.zeros((frames,2))
    lowF = mX[:,index0K:indexUnder3K+1]
    hiF = mX[:,indexOver3K:index10K+1]

    lowFeng = 10*np.log10(np.sum(lowF,1))
    hiFeng = 10*np.log10(np.sum(hiF,1))
